...
func (e *endpoint) WritePacket(r *stack.Route, params stack.NetworkHeaderParams, pkt *stack.PacketBuffer) tcpip.Error {
	...
	return e.writePacket(r, pkt, params.Protocol, false /* headerIncluded */, params.FromRawSocket)
}

func (e *endpoint) writePacket(r *stack.Route, pkt *stack.PacketBuffer, protocol tcpip.TransportProtocolNumber, headerIncluded bool, fromRawSocket bool) tcpip.Error {
	if r.Loop()&stack.PacketLoop != 0 {
		...
		e.handleLocalPacket(pkt, !headerIncluded && !fromRawSocket /* canSkipRXChecksum */)
	}
...
func (e *endpoint) WriteHeaderIncludedPacket(r *stack.Route, pkt *stack.PacketBuffer) tcpip.Error {
	...
	return e.writePacket(r, pkt, proto, true /* headerIncluded */, false /* fromRawSocket */)
}
...
func (e *endpoint) forwardPacketWithRoute(route *stack.Route, pkt *stack.PacketBuffer) ip.ForwardingError {
	...
	switch err := forwardToEp.writePacket(route, newPkt, newPkt.TransportProtocolNumber, true /* headerIncluded */, false /* fromRawSocket */); err.(type) {
	case nil:
		return nil
...
// checkChecksumIntegrity verifies the checksum of the packet.
//
// It matches Linux include/net/netfilter/nf_reject.h:nf_reject_verify_csum().
func checkChecksumIntegrity(pkt *stack.PacketBuffer, src, dst tcpip.Address) bool {
	...
	case header.TCPProtocolNumber:
		...
		var origTCPHdr header.TCP
		var payloadLength uint16
		var payloadChecksum uint16

		if len(th) >= header.TCPMinimumSize {
			origTCPHdr = header.TCP(th)
			payloadLength = uint16(pkt.Data().Size())
			payloadChecksum = pkt.Data().Checksum()
		} else if len(th) == 0 {
			// Extract TCP header from pkt.Data()
			h, ok := pkt.Data().PullUp(header.TCPMinimumSize)
			if !ok {
				return false
			}
			tcpHdr := header.TCP(h)
			hdrLen := int(tcpHdr.DataOffset())
			if hdrLen < header.TCPMinimumSize || pkt.Data().Size() < hdrLen {
				return false
			}
			thSlice, ok := pkt.Data().PullUp(hdrLen)
			if !ok {
				return false
			}
			origTCPHdr = header.TCP(thSlice)
			payloadLength = uint16(pkt.Data().Size() - hdrLen)
			dataStart := pkt.ReservedHeaderBytes() + pkt.HeaderSize()
			payloadChecksum = pkt.Data().ChecksumAtOffset(dataStart + hdrLen)
		} else {
			return false
		}
		return origTCPHdr.IsChecksumValid(src, dst, payloadChecksum, payloadLength)

	case header.UDPProtocolNumber:
		th := pkt.TransportHeader().Slice()
		var origUDPHdr header.UDP
		var payloadChecksum uint16

		if len(th) >= header.UDPMinimumSize {
			origUDPHdr = header.UDP(th)
			payloadChecksum = pkt.Data().Checksum()
		} else if len(th) == 0 {
			...
			origUDPHdr = header.UDP(h)
			dataStart := pkt.ReservedHeaderBytes() + pkt.HeaderSize()
			payloadChecksum = pkt.Data().ChecksumAtOffset(dataStart + header.UDPMinimumSize)
		} else {
			return false
		}
		return origUDPHdr.IsChecksumValid(src, dst, payloadChecksum)

	case header.ICMPv6ProtocolNumber:
		th := pkt.TransportHeader().Slice()
		var origICMPv6Hdr header.ICMPv6
		var payloadCsum uint16
		var payloadLen int

		if len(th) >= header.ICMPv6MinimumSize {
			origICMPv6Hdr = header.ICMPv6(th)
			...
			origICMPv6Hdr = header.ICMPv6(h)
			dataStart := pkt.ReservedHeaderBytes() + pkt.HeaderSize()
			payloadCsum = pkt.Data().ChecksumAtOffset(dataStart + header.ICMPv6MinimumSize)
			payloadLen = pkt.Data().Size() - header.ICMPv6MinimumSize
		} else {
			return false
		}
		got, want := origICMPv6Hdr.Checksum(), header.ICMPv6Checksum(header.ICMPv6ChecksumParams{
			Header:      origICMPv6Hdr,
			Src:         src,
			Dst:         dst,
...
func (p *protocol) SendRejectionError(pkt *stack.PacketBuffer, rejectWith stack.RejectIPv6WithICMPType, inputHook bool) tcpip.Error {
	...
	if len(origIPHdr) < header.IPv6MinimumSize {
		return nil
	}
	// Do not send rejection responses for fragmented packets.
	// Matches Linux net/ipv6/exthdrs_core.c:ipv6_skip_exthdr()
	networkHeader := pkt.NetworkHeader().Slice()
	extHeadersBuf := buffer.MakeWithData(networkHeader[header.IPv6MinimumSize:])
	dataBuf := pkt.Data().ToBuffer()
	extHeadersBuf.Merge(&dataBuf)
	// MakeIPv6PayloadIterator takes ownership of extHeadersBuf, so it will be released via it.Release() below.
	it := header.MakeIPv6PayloadIterator(header.IPv6ExtensionHeaderIdentifier(origIPHdr.NextHeader()), extHeadersBuf)
	isFragment := false
	transProto := tcpip.TransportProtocolNumber(origIPHdr.NextHeader())
	for {
		extHdr, done, err := it.Next()
		if err != nil || done {
			break
		}
		if frag, ok := extHdr.(header.IPv6FragmentExtHdr); ok {
		...
		if rawPayload, ok := extHdr.(header.IPv6RawPayloadHeader); ok {
			transProto = tcpip.TransportProtocolNumber(rawPayload.Identifier)
			rawPayload.Release()
			break
		}
		extHdr.Release()
	...
	pkt.TransportProtocolNumber = transProto

	origIPHdrSrc := origIPHdr.SourceAddress()
	origIPHdrDst := origIPHdr.DestinationAddress()

	// Broadcast/Multicast Handling.
	if header.IsV6MulticastAddress(origIPHdrSrc) || header.IsV6MulticastAddress(origIPHdrDst) ||
		pkt.PktType == tcpip.PacketBroadcast || pkt.PktType == tcpip.PacketMulticast ||
		origIPHdrSrc == header.IPv6Any || origIPHdrDst == header.IPv6Any {
		return nil
	}

	// ICMPv6 Destination Unreachable Loop Prevention.
	// Matches Linux net/ipv6/netfilter/nf_reject_ipv6.c:nf_send_unreach6() / nf_skb_is_icmp6_unreach()
	if pkt.TransportProtocolNumber == header.ICMPv6ProtocolNumber {
		var icmpType header.ICMPv6Type
		th := pkt.TransportHeader().Slice()
		if len(th) >= header.ICMPv6MinimumSize {
			icmpType = header.ICMPv6(th).Type()
			if icmpType == header.ICMPv6DstUnreachable {
				return nil
			}
		} else if len(th) == 0 {
	...
	if !checkChecksumIntegrity(pkt, origIPHdrSrc, origIPHdrDst) {
		return nil
	}
	...
	case stack.RejectIPv6WithTCPReset:
		return p.sendTCPReset(pkt, inputHook)
	case stack.RejectIPv6WithICMPNotNeighbour:
		return p.returnError(&icmpReasonBeyondScope{}, pkt, inputHook)
	case stack.RejectIPv6WithICMPPolicyFail:
		return p.returnError(&icmpReasonPolicy{}, pkt, inputHook)
	case stack.RejectIPv6WithICMPRejectRoute:
		return p.returnError(&icmpReasonRejectRoute{}, pkt, inputHook)
	default:
		panic(fmt.Sprintf("unhandled %[1]T = %[1]d", rejectWith))
	}
}

// sendTCPReset sends a TCP reset in response to the packet.
//
// Matches Linux net/ipv6/netfilter/nf_reject_ipv6.c:nf_send_reset6()
func (p *protocol) sendTCPReset(pkt *stack.PacketBuffer, inputHook bool) tcpip.Error {
	...
	// Ensure the transport protocol is TCP.
	...
	if len(th) >= header.TCPMinimumSize {
		...
		origTCPHdr = header.TCP(thSlice)
		payloadLen = uint32(pkt.Data().Size() - hdrLen)
	} else {
		return nil
	}

	// 1. TCP RST Loop Prevention.
	if origTCPHdr.Flags()&header.TCPFlagRst != 0 {
		return nil
	}

	// 2. Unicast Address Validation.
	// As per Linux net/ipv6/netfilter/nf_reject_ipv6.c:nf_send_reset6()
	// Both source and destination must be unicast addresses (not multicast or unspecified).
	origIPHdrSrc := origIPHdr.SourceAddress()
	origIPHdrDst := origIPHdr.DestinationAddress()
	if header.IsV6MulticastAddress(origIPHdrSrc) || header.IsV6MulticastAddress(origIPHdrDst) ||
		pkt.PktType == tcpip.PacketBroadcast || pkt.PktType == tcpip.PacketMulticast ||
	...
	if !pkt.RXChecksumValidated {
		...
		if len(th) >= header.TCPMinimumSize {
			...
			dataStart := pkt.ReservedHeaderBytes() + pkt.HeaderSize()
			hdrLen := int(origTCPHdr.DataOffset())
			payloadChecksum = pkt.Data().ChecksumAtOffset(dataStart + hdrLen)
		}
		if !origTCPHdr.IsChecksumValid(origIPHdrSrc, origIPHdrDst, payloadChecksum, uint16(payloadLen)) {
			return nil
		}
	}
	...
	var seq uint32
	var ack uint32
	var flags header.TCPFlags = header.TCPFlagRst

	if origTCPHdr.Flags()&header.TCPFlagAck != 0 {
		seq = origTCPHdr.AckNumber()
	} else {
		var synFin uint32
		...
		ack = origTCPHdr.SequenceNumber() + payloadLen + synFin
		flags |= header.TCPFlagAck
	}

	tcpPkt := stack.NewPacketBuffer(stack.PacketBufferOptions{
		ReserveHeaderBytes: int(route.MaxHeaderLength()) + header.TCPMinimumSize,
	})
	defer tcpPkt.DecRef()
	...
	tcpHdr.SetSourcePort(origTCPHdr.DestinationPort())
	tcpHdr.SetDestinationPort(origTCPHdr.SourcePort())
	tcpHdr.SetSequenceNumber(seq)
	tcpHdr.SetAckNumber(ack)
	tcpHdr.SetFlags(uint8(flags))
	tcpHdr.SetDataOffset(header.TCPMinimumSize)

	// Compute TCP checksum.
	xsum := header.PseudoHeaderChecksum(header.TCPProtocolNumber, route.LocalAddress(), route.RemoteAddress(), header.TCPMinimumSize)
	tcpHdr.SetChecksum(0)
	tcpHdr.SetChecksum(^tcpHdr.CalculateChecksum(xsum))

	// Omit mark propagation as gVisor doesn't support socket/packet marks on PacketBuffer.
	// (Matches Linux net/ipv6/netfilter/nf_reject_ipv6.c:nf_send_reset6())
