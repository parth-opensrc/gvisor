...
func (e *endpoint) WritePacket(r *stack.Route, params stack.NetworkHeaderParams, pkt *stack.PacketBuffer) tcpip.Error {
	...
	return e.writePacket(r, pkt, params.FromRawSocket)
}

func (e *endpoint) writePacket(r *stack.Route, pkt *stack.PacketBuffer, fromRawSocket bool) tcpip.Error {
	...
	return e.writePacketPostRouting(r, pkt, false /* headerIncluded */, fromRawSocket)
}

func (e *endpoint) writePacketPostRouting(r *stack.Route, pkt *stack.PacketBuffer, headerIncluded bool, fromRawSocket bool) tcpip.Error {
...
func (e *endpoint) WriteHeaderIncludedPacket(r *stack.Route, pkt *stack.PacketBuffer) tcpip.Error {
	...
	return e.writePacketPostRouting(r, pkt, true /* headerIncluded */, false /* fromRawSocket */)
}
...
func (e *endpoint) forwardPacketWithRoute(route *stack.Route, pkt *stack.PacketBuffer, updateOptions bool) ip.ForwardingError {
	...
	switch err := forwardToEp.writePacketPostRouting(route, newPkt, true /* headerIncluded */, false /* fromRawSocket */); err.(type) {
	case nil:
		return nil
...
func checkChecksumIntegrity(pkt *stack.PacketBuffer, src, dst tcpip.Address) bool {
	...
	case header.ICMPv4ProtocolNumber:
		...
		if len(th) >= header.ICMPv4MinimumSize {
			...
			if pkt.Data().Size() < header.ICMPv4MinimumSize {
				return false
			}
			return pkt.Data().Checksum() == 0xffff
		} else {
			return false
		}
		...
		return checksum.Checksum(th, payloadCsum) == 0xffff
	}
	return true
}
...
func (p *protocol) SendRejectionError(pkt *stack.PacketBuffer, rejectWith stack.RejectIPv4WithICMPType, inputHook bool) tcpip.Error {
	...
	if len(origIPHdr) < header.IPv4MinimumSize {
		return nil
	}
	// Do not send rejection responses for fragmented packets.
	// Matches Linux net/ipv4/netfilter/nf_reject_ipv4.c:nf_reject_ip_tcphdr_get()
	if origIPHdr.FragmentOffset() != 0 {
		return nil
	}

	pkt.TransportProtocolNumber = tcpip.TransportProtocolNumber(origIPHdr.Protocol())

	origIPHdrSrc := origIPHdr.SourceAddress()
	origIPHdrDst := origIPHdr.DestinationAddress()

	// Broadcast/Multicast Handling.
	if header.IsV4MulticastAddress(origIPHdrSrc) || header.IsV4MulticastAddress(origIPHdrDst) ||
		pkt.NetworkPacketInfo.LocalAddressBroadcast ||
		pkt.PktType == tcpip.PacketBroadcast || pkt.PktType == tcpip.PacketMulticast ||
		origIPHdrSrc == header.IPv4Any || origIPHdrDst == header.IPv4Any {
		return nil
	}

	// ICMP Destination Unreachable Loop Prevention.
	// Matches Linux net/ipv4/netfilter/nf_reject_ipv4.c:nf_send_unreach() / nf_skb_is_icmp_unreach()
	if pkt.TransportProtocolNumber == header.ICMPv4ProtocolNumber {
		var icmpType header.ICMPv4Type
		th := pkt.TransportHeader().Slice()
		if len(th) >= header.ICMPv4MinimumSize {
			icmpType = header.ICMPv4(th).Type()
			if icmpType == header.ICMPv4DstUnreachable {
				return nil
			}
		} else if len(th) == 0 {
			b, ok := pkt.Data().PullUp(header.ICMPv4MinimumSize)
			if ok {
				icmpType = header.ICMPv4(b).Type()
				if icmpType == header.ICMPv4DstUnreachable {
					return nil
				}
			}
...
//
// Matches Linux net/ipv4/netfilter/nf_reject_ipv4.c:nf_send_reset()
func (p *protocol) sendTCPReset(pkt *stack.PacketBuffer, inputHook bool) tcpip.Error {
	origIPHdr := header.IPv4(pkt.NetworkHeader().Slice())
	if len(origIPHdr) < header.IPv4MinimumSize {
		return nil
	}
	if origIPHdr.Protocol() != uint8(header.TCPProtocolNumber) {
		return nil
	}
	...
	if !pkt.RXChecksumValidated {
		...
		if !origTCPHdr.IsChecksumValid(origIPHdr.SourceAddress(), origIPHdr.DestinationAddress(), payloadChecksum, uint16(payloadLen)) {
			return nil
		}
	}
